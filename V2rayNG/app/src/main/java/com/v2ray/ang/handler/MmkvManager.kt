...
object MmkvManager {
    ...
    /**
     * Adds an upload/download byte delta (measured since the last stats query) to a server's
     * cumulative traffic total. Called periodically while that server is the active connection,
     * so the totals persist across app restarts and reconnects.
     */
    fun addServerTraffic(guid: String, uploadDeltaBytes: Long, downloadDeltaBytes: Long) {
        if (guid.isBlank() || (uploadDeltaBytes <= 0L && downloadDeltaBytes <= 0L)) {
            return
        }
        val aff = decodeServerAffiliationInfo(guid) ?: ServerAffiliationInfo()
        aff.trafficUploadBytes += uploadDeltaBytes
        aff.trafficDownloadBytes += downloadDeltaBytes
        serverAffStorage.encode(guid, JsonUtil.toJson(aff))
    }
