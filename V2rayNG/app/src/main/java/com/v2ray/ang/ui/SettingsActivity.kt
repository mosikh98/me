...
fun SettingsScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            val accentColor by com.v2ray.ang.compose.AccentColorManager.accentColor.collectAsStateWithLifecycle()
            var showColorPicker by remember { mutableStateOf(false) }
            SettingsMenuItem(
                title = stringResource(R.string.title_pref_theme_color),
                subtitle = stringResource(R.string.summary_pref_theme_color),
                onClick = { showColorPicker = true }
            )
            if (showColorPicker) {
                com.v2ray.ang.compose.AccentColorPickerDialog(
                    currentColor = accentColor,
                    onDismiss = { showColorPicker = false },
                    onColorSelected = {
                        com.v2ray.ang.compose.AccentColorManager.setColor(it)
                        showColorPicker = false
                    },
                    onReset = {
                        com.v2ray.ang.compose.AccentColorManager.reset()
                        showColorPicker = false
                    }
                )
            }
